# case14: PR_tool to 3DBlox package

Load this package from its directory with:

```tcl
source load_case14.tcl
```

`case14.3dbx` and its included `.3dbv` are standard 3DBlox.  The top
...
plus bump maps only; `case14_interposer.def` and
`case14_interposer_macros.lef` are still emitted for later drawing but
