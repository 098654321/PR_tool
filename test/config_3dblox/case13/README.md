# case13: PR_tool to 3DBlox package

Load this package from its directory with:

```tcl
source load_case13.tcl
```

`case13.3dbx` and its included `.3dbv` are standard 3DBlox.  The top
...
plus bump maps only; `case13_interposer.def` and
`case13_interposer_macros.lef` are still emitted for later drawing but
