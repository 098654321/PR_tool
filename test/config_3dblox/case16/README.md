# case16: PR_tool to 3DBlox package

Load this package from its directory with:

```tcl
source load_case16.tcl
```

`case16.3dbx` and its included `.3dbv` are standard 3DBlox.  The top
...
plus bump maps only; `case16_interposer.def` and
`case16_interposer_macros.lef` are still emitted for later drawing but
