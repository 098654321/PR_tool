# case10: PR_tool to 3DBlox package

Load this package from its directory with:

```tcl
source load_case10.tcl
```

`case10.3dbx` and its included `.3dbv` are standard 3DBlox.  The top
...
plus bump maps only; `case10_interposer.def` and
`case10_interposer_macros.lef` are still emitted for later drawing but
