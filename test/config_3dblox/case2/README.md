# case2: PR_tool to 3DBlox package

Load this package from its directory with:

```tcl
source load_case2.tcl
```

`case2.3dbx` and its included `.3dbv` are standard 3DBlox.  Its external
...
adjacent `case2_connectivity.v` instead.  That connectivity is standard
...
interposer external I/O positions are read directly from its DEF pins.  The original PR_tool configuration is copied
