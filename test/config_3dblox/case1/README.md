# case1: PR_tool to 3DBlox package

Load this package from its directory with:

```tcl
source load_case1.tcl
```

`case1.3dbx` and its included `.3dbv` are standard 3DBlox.  The top
...
edge margin of 300 microns.  This case uses a COB array of 9 x 12,
yielding an interposer size of 7300.0 x 5500.0 microns.
...
plus bump maps only; `case1_interposer.def` and
`case1_interposer_macros.lef` are still emitted for later drawing but
