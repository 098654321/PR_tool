# case8: PR_tool to 3DBlox package

Load this package from its directory with:

```tcl
source load_case8.tcl
```

`case8.3dbx` and its included `.3dbv` are standard 3DBlox.  The top
...
plus bump maps only; `case8_interposer.def` and
`case8_interposer_macros.lef` are still emitted for later drawing but
