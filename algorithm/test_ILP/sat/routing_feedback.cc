...
auto solve_with_feedback(
    ...
    for (std::size_t round = 0; round < options.max_feedback_rounds; ++round) {
        ...
        auto cadical_options = options.cadical;
        if (cadical_options.enable_sat_log)
            cadical_options.log_dir += "/round_" + std::to_string(round);
        auto session = CadicalSession {cadical_options};
