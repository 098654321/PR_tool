...
- 可达性阶段：根据 record 的 bbox 与 `rho[record_id]` 预计算 end track 到 start track 的候选关系。
- MCF 阶段：可选启用 Gurobi，在 track 级图上求解 BusMCF 和 SimpleMCF。
- 诊断阶段：可选用 maze-check 或 simple-maze 判断 MCF 失败是否来自建模/容量限制。

该子工程的硬件和电路基础来自项目根目录下的 `source/hardware` 与 `source/circuit`。算法依据优先参考 `algorithm/test_ILP/problem_formulation/` 中的方法文档，尤其是当前实现对应的“第六版方法与分析（SAT1）”。

## 工作流程中必须要做的事情

- 改代码前先读清相关方法文档、硬件映射和现有实现，不要凭记忆改模型。其中方法文档在`problem_formulation/`，硬件映射在"PR_tool根目录/source/hardware"和"PR_tool根目录/source/circuit"中。注意，不允许修改方法文档。
- 中到大规模修改后，需要评估是否同步更新本文件，以及是否在项目根目录 `.plan/` 下新增或更新改动记录。同时启动一个新的子agent，评估需要修改的内容是否完整、正确的完成
...
├── common/                 # record、SAT/MCF 结果、bbox rho 状态等共用类型
├── sat_allocation/         # TOB SAT 编码、CaDiCal 封装、SAT-only 与 SAT+MCF pipeline
├── precompute/             # bbox、reach、k-shortest、legacy ILP reach 预计算
...
- `common/tob_allocation_types.hh` 与 `common/tob_bbox_expansion.hh`
  - `TobIlpResult` 保存 SAT 分配结果、track endpoint、`bbox_expand_by_record`。
  - `TobBBoxExpansionState` 保存 per-record `rho[record_id]`，当前上限固定为 4。
...
- `precompute/`
  ...
  - `tob_reach_with_range.*` 根据 `rho` 扩展 start track 候选。
  - `tob_channel_kshortest.*` 在单 COBUnit 隔离图内寻找补充候选 track。
  - `ilp_reach_precompute.*` 服务 legacy ILP reach 与 base reach。

- `mcf/`
  - `cob_mcf_router.*` 构建 track 级全局图，准备 commodity，求解 BusMCF 与 SimpleMCF。
  - `mcf_bbox.*` 根据 SAT 成功轮的 `bbox_expand_by_record` 构造 MCF 可行图。
...
- `-v` / `-vv`：增加日志详细程度；`-v` 会打印 SAT reach 中的 bbox、end track、start tracks。
...
```bash
./output/test_ILP test/config/case7 --enable-mcf-routing --enable-mcf-obj
```

涉及局部 bbox / k-shortest / SAT reach 的改动，优先复跑近期关注样例：

```bash
./output/test_ILP test/config/case10
./output/test_ILP test/config/case11
./output/test_ILP test/config/case12
./output/test_ILP test/config/case13
```
...
- `rho[record_id]` 是局部 bbox 扩展量；`range_level` 只作为 `max(rho)` 的兼容日志摘要，不应作为 MCF 真实范围来源。
- 默认日志不打印完整 `rho_by_record` 数组；需要定位局部扩展时优先看 `max_rho`、`changed_records`、`fail_set`。
...
- PNnet 在 MCF 中继续不裁剪；它的 `rho` 只影响 SAT 阶段候选 start tracks。
- track 只能在所属 COBUnit 内连通；k-shortest 不允许跨 COBUnit 搜索。
- 新日志要包含足够定位信息，例如 `record_id`、`origin_key`、`bit_id`、`rho`、`bus_key`、unit、bbox。
