    ...
    for (const auto& net : nets) {
        for (const auto& demand : net.demands) {
            ...
            if (demand.candidate_source_indices.size() != 1) {
                throw std::invalid_argument(std::format(
                    "net '{}' demand {} requires exactly one candidate source, got {}",
                    net.name,
                    demand.demand_id,
